    ...
    <div style={profilePage} className="no-scrollbar">
      <div style={profileStickyTop}>
        <div style={profileHeader}>
          ...
          <div style={profileTitleBlock}><h1 style={profileTitle}>{profile?.display_name || "Your"}'s Looptie</h1><p style={profileSubtitle}>Your saved media, spaces, and favorites.</p></div>
          <button style={settingsButton} onClick={() => setTab("settings")}><Settings size={20} /></button>
        </div>
        <div style={profileStats}>
          <div style={profileStatCard}><strong style={profileStatNumber}>{items.length}</strong><span style={profileStatLabel}>Items</span></div>
...
const videoBadge = { position: "absolute", top: "10px", right: "10px", width: "32px", height: "32px", borderRadius: "999px", background: "rgba(0,0,0,.65)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 3 };