...
export default function FeedMembersModal({ feed, onClose, onMembershipChange }) {
  const [people, setPeople] = useState([]);
  const [loading, setLoading] = useState(true);

  const loadPeople = async () => {
    const { data, error } = await supabase.rpc("get_feed_people", { target_feed_id: feed.id });
    if (error) console.error("Could not load Feed access:", error);
    else {
      setPeople(data || []);
      onMembershipChange?.(feed.id, (data || []).some((person) => person.role !== "owner"));
    }
    setLoading(false);
  };
  ...
  const removePerson = async (person) => {
    ...
    setPeople(remaining);
    onMembershipChange?.(feed.id, remaining.some((entry) => entry.role !== "owner"));
  };

  return <div style={overlay} onClick={onClose}>
    <div style={card} onClick={(event) => event.stopPropagation()}>
      <div style={header}>
        <div><div style={eyebrow}>COLLABORATIVE FEED</div><h2 style={title}>Manage access</h2></div>
        <button type="button" style={iconButton} onClick={onClose} aria-label="Close"><X size={20} /></button>
      </div>
      <p style={copy}>See who can add to {feed.name} and remove access when needed.</p>

      <div style={peopleHeader}><Users size={18} /><span>Feed access</span></div>
...
const peopleHeader = { display:"flex", alignItems:"center", gap:"8px", fontWeight:"var(--weight-semibold)", margin:"0 0 10px" };
