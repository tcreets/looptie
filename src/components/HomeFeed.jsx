        ...
        {sortedFeedItems.map((item) => (
          <div key={item.id} style={feedCard}>
            ...
            <div style={floatingActions}>
              {(item.media_type === "video" || (item.media_type === "link" && getInstagramEmbedUrl(item.source_url) && item.media_url)) && <>
...
const videoStageStyle = { width:"100%", height:"100%", position:"relative", overflow:"hidden", background:"var(--surface)" };
const videoPosterStyle = { position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"cover", filter:"blur(18px)", transform:"scale(1.06)", opacity:.55 };
const videoStyle = { ...imageStyle, position:"relative", zIndex:1, objectFit:"contain", background:"#000", transition:"opacity 120ms ease" };
