...
export default function AuthScreen({ setUser }) {
  ...
  const handleAuth = async () => {
    const { data, error } = mode === "login" ? await supabase.auth.signInWithPassword({ email, password }) : await supabase.auth.signUp({ email, password, options: { emailRedirectTo: window.location.origin } });
    console.log("AUTH DATA:", data); console.log("AUTH ERROR:", error);
    if (error) { console.error("AUTH ERROR:", error); alert("Auth failed: " + error.message); return; }
    if (mode === "signup") { alert("The email you signed up with should receive a confirmation email. Check your inbox or spam folder and confirm your account before logging in."); setMode("login"); return; }
    if (data.user) { setUser(data.user); alert("Logged in!"); }
  };

  return <div style={authPage}>
    <h1 style={{ color: "var(--text-primary)" }}>Welcome to Looptie</h1>
    <p style={subtitle}>Your private space for inspiration, memories, routines, and the things you actually want to revisit.</p>
    ...
    <button style={{ ...primaryButton, opacity: !email.trim() || !password.trim() ? 0.45 : 1, cursor: !email.trim() || !password.trim() ? "not-allowed" : "pointer" }} disabled={!email.trim() || !password.trim()} onClick={handleAuth}>{mode === "login" ? "Log In" : "Create Account"}</button>
    {mode === "login" && <button style={forgotButton} onClick={async () => {
      if (!email.trim()) { alert("Enter your email first."); return; }
      const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: window.location.origin });
      if (error) { if (error.message.toLowerCase().includes("email not confirmed")) { alert("Your account hasn't been confirmed yet. Check the inbox or spam folder for the email you signed up with."); return; } alert(error.message); return; }
      alert("If an account exists for this email, a password reset link has been sent.");
    }}>Forgot Password?</button>}
    <button style={switchButton} onClick={() => setMode(mode === "login" ? "signup" : "login")}>{mode === "login" ? "Need an account? Create one" : "Already have an account? Log in"}</button>
  </div>;
}
